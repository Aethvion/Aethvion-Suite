...
async function pollStartupStatus() {
    ...
    if (splashVersion) {
        fetch('/api/system/version-info?v=' + Date.now())
            ...
            .then(data => {
                if (data && data.local && data.local.version) {
                    splashVersion.textContent = data.local.version;
                }
...
function goToAIHub() {
    const _noSidebar = new Set(['chat', 'agents', 'aethviondb', 'automate',
...
async function launchModule(name) {
    try {
        ...
        if (port) {
            ...
            const defaults = { 'photo': 8083, 'audio': 8083, 'driveinfo': 8084, 'finance': 8085, 'kanban': 8089 };
...
async function updateModuleStatusBadges() {
    try {
        ...
        ['photo', 'audio', 'driveinfo', 'finance', 'kanban'].forEach(mod => {
